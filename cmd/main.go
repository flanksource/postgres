...
func runPgTune(cmd *cobra.Command, args []string) error {
	...
	// Apply overrides
	if memoryGB > 0 {
		sysInfo.TotalMemoryBytes = uint64(memoryGB * float64(GB))
	}
	if cpus > 0 {
...
func generateConfigSnippet(sysInfo *sysinfo.SystemInfo, params *pgtune.TunedParameters, dbType sysinfo.DBType) string {
	...
	snippet.WriteString(fmt.Sprintf("# System: %.1f GB RAM, %d CPUs\n",
		float64(sysInfo.TotalMemoryBytes)/float64(GB), sysInfo.CPUCount))
...
func runAutoStart(cmd *cobra.Command, args []string) error {
	...
	if pgTune {
		...
		if err := runPGTuneOptimization(postgres); err != nil {
...
// runPGTuneOptimization runs pg_tune and saves the optimized configuration
func runPGTuneOptimization(postgres *server.Postgres) error {
	// Detect system information
	sysInfo, err := sysinfo.DetectSystemInfo()
	if err != nil {
		return fmt.Errorf("failed to detect system info: %w", err)
	...
	// Calculate optimal configuration
	maxConnections := pgtune.GetRecommendedMaxConnections(sysinfo.DBTypeWeb)
	tuningConfig := &pgtune.TuningConfig{
		SystemInfo:     sysInfo,
		MaxConnections: maxConnections,
		DBType:         sysinfo.DBTypeWeb,
	...
	// Save to postgresql.conf in data directory
	dataDir := getDataDir()
	if dataDir == "" {
		return fmt.Errorf("data directory not detected")
	}

	configPath := filepath.Join(dataDir, "postgresql.auto.conf")
	if err := os.WriteFile(configPath, []byte(configContent), 0644); err != nil {
		return fmt.Errorf("failed to write postgresql.auto.conf: %w", err)
	}

	fmt.Printf("Saved optimized configuration to: %s\n", configPath)
